...
public class Board extends View {
    ...
    public void setMachine(Machine g) {
        machine = g;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!machine.isEnded()  &&  event.getAction() == MotionEvent.ACTION_DOWN) {
            int x = (int) (event.getX() / eltW);
            int y = (int) (event.getY() / eltH);
            char win = machine.play(x, y);
            invalidate();
    ...
    private void drawBoard(Canvas canvas) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                drawElt(canvas, machine.getmah(i, j), i, j);
            }
        }
    }
